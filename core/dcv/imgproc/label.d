...
/+ // keep this here for now. TODO: convex hull and ellipse fitting
...
in
{
    ...
    assert(Method == 1 || Method == 2 || Method == 3, "Method must be one of 1, 2, and 3");
}
do
{
    static if(Method == 1){
    ...
    static if(Method == 3){ // light speed :)
        LabelizerV3 lblzr;
    }
...
private struct LabelizerV1(SliceT)
{
    ...
    private void dfs(alias Conn)(immutable ulong i, immutable ulong j, immutable ulong current_label, const ref SliceT img)
    {
        ...
        while(!stack.empty){
            ...
            immutable ulong arg_j = stack[].front; stack.removeFront();
            immutable ulong arg_i = stack[].front; stack.removeFront();
    ...
    auto labelize(alias Conn, SliceKind kind)(in Slice!(ubyte*, 2LU, kind) img)
    {
